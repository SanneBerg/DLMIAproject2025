...
HEADER = ["Name",
          "Dice LV", "Volume LV", "Err LV(ml)", "HD95 LV",
          "Dice RV", "Volume RV", "Err RV(ml)", "HD95 RV",
          "Dice MYO", "Volume MYO", "Err MYO(ml)", "HD95 MYO"]
...
def metrics(img_gt, img_pred, voxel_size):
    ...
    for c in [3, 1, 2]:
        ...
        hausdorff = hd95(gt_c_i, pred_c_i)
...
def compute_metrics_on_files(path_gt, path_pred):
    ...
    formatting = "{:>14}, {:>7}, {:>9}, {:>10}, {:>7}, {:>9}, {:>10}, {:>6}, {:>7}, {:>9}, {:>6}, {:>7}, {:>9}, {:>6}"
...
def compute_metrics_on_directories(dir_gt, dir_pred):
    ...
    for p_gt, p_pred in zip(lst_gt, lst_pred):
        ...
        gt_base = os.path.basename(p_gt).replace('_gt', '').replace('_segmentation', '')
        pred_base = os.path.basename(p_pred).replace('_gt', '').replace('_segmentation', '')
    ...
    df.to_csv("results{}.csv".format(time.strftime("%Y%m%d_%H%M%S")), index=False)
